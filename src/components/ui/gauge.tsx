...
export const Gauge = ({ value, max, weather, unit, name, label }: GaugeProperties) => {
  ...
  return (
    <div className="flex flex-col justify-center items-center">
      <p className="text-xs text-center font-medium mb-4">{name}</p>
      <div className="size-24 relative">
        <svg xmlns="http://www.w3.org/2000/svg" width="96" height="96" viewBox="0 0 96 96" fill="none" className="absolute">
          ...
          {max !== -1 && typeof value === "number" && <circle cx={dot.x} cy={dot.y} z="10" r="6" fill="A3A3A3" stroke="black" strokeWidth="4" />}
        </svg>
        <div className="size-full flex flex-col justify-center items-center">
          <div>
            {!weather && <p className="text-3xl text-neutral-400 font-bold text-center leading-none tracking-tighter m-0">{value}</p>}
